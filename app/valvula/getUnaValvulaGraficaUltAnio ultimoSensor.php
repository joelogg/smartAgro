...
//Este archivo nos da la grafica del dia actual
    ...
    $sql = "SELECT IdCam FROM campovalvula WHERE IdVal='".$idVal."'";
    ...
    $sql = "SELECT nodoterminal.IdNodTer, CanSenNodTer FROM camponodoterminal, nodoterminal WHERE camponodoterminal.IdNodTer=nodoterminal.IdNodTer AND IdCam='".$idCam."'";
    $result = $conexion->query($sql);
    if ($result->num_rows > 0) 
    {
        while($row = $result->fetch_assoc()) 
